...
<script>
    ...
    if (jenis == 'harian') {
        title = "LAPORAN JUMLAH PENGGUNAAN OBAT " + "{{ tanggal($start, null, true) }}" + " - " +
            "{{ tanggal($end, null, true) }}" + " PT MANDIRI INTI PERKASA";
    } else if (jenis == 'bulanan') {
        var set_year = "{{ $set_year }}";
        title = "LAPORAN JUMLAH PENGGUNAAN OBAT TAHUN " + set_year +
    ...
    var options = {
        ...
        scales: {
            y: {
                ...
                title: {
                    display: true,
                    text: 'Total Penggunaan (satuan obat)'
                },
                ticks: {
                    stepSize: 10
                }
    ...
    var datasets = [{
        label: 'Total Penggunaan (satuan obat)',
        data: data,
        parsing: {
            yAxisKey: 'total',
            xAxisKey: 'nama_obat',
