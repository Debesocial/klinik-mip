...
<script>
    ...
    if (jenis == 'bulanan') {
        set_year = "{{ $set_year }}";
        title = "STATISTIK LAPORAN KESEHATAN KERJA TAHUN " + set_year + " " + namaperusahaan ??
        ...
        title = "STATISTIK LAPORAN KESEHATAN KERJA TAHUN " + "{{ tanggal($start, null, true) }}" + " - " +
            "{{ tanggal($end, null, true) }}" + " " + namaperusahaan ?? "PT MANDIRI INTI PERKASA";
    ...
    newKunjungan.forEach(data => {
        ...
        if (jenis == 'harian') {
            header.push({
                title: cekSingle(newDate.getDate()) + '-' + cekSingle(newDate.getMonth() + 1) +
                    '-' + newDate.getFullYear()
            });
        } else if (jenis == 'bulanan') {
            header.push({
                title: listBulan[newDate.getMonth()]
            });
        }
